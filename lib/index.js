 ...
 * updata time 20211223 --上移50px，这个应该改成动态计算半个滚动单位高度
 * @animationDirection boolean --滚动方向，默认从下到上，从左到右
 * @animationTime number --过场时间 单位：S
 * @bigNum string --默认100，不用设置，防止内存过大
 ...
 */
 ...
 const RollFree = ({
     ...
     const eventHandle = () => {
       ...
       if (document.hidden) {
         ...
         !running.current &&
           ...
           }, time - ((new Date().getTime() - runTime.current) % time))); // 精度损失
       }
     };
   ...
   useEffect(() => {
    ...
    if (!children.length) children = [<div />];
     let ends = [];
   ...
   const timeOut = (muchtime) => {
     ...
     clear();
     if (runTime.current < 0) runTime.current += onceTime.current; // 精度损失
     const raf = () => {
       clear();
   ...
   const clear = () => {
     if (timers.current) {
       cancelAnimationFrame(timers.current);
   ...
   const pause = () => {
     ...
     // 暂停停止计时
     timer.current = (timer.current || time) - (nowTime - onceTime.current); // 精度损失
     runTime.current -= nowTime; // 精度损失
     clear();
   };
   // 装载Node
   const renderNode = (ty) => {
     ...
     // setTimeout执行时间纠正回调
     const nowTime = new Date().getTime();
     let jsTimersFixed = (nowTime - runTime.current) % time; // 精度损失
     if (!running.current) {
       order.current = Math.floor((nowTime - runTime.current) / time) % 2 ? 0 : 1;
       // console.log('切换！！！！', (nowTime - runTime.current) / time, jsTimersFixed);
     }
     // 页面不可见时间判断切换order,onceTime.current是延迟发生后的时间
     // if (!(running.current || Math.floor((nowTime - onceTime.current + jsTimersFixed) / time) % 2)) {
     //   order.current = order.current ? 0 : 1;
     //   console.log('切换！！！！', (nowTime - onceTime.current + jsTimersFixed) / time, jsTimersFixed);
     // }
     ...
     // );
     // 首次加载防止重叠time置后
     timeOut(first.current ? time + 48 : time - jsTimersFixed);
     first.current = false;
   };
       ...
       <style jsx="true">
         {`
            .rollfree-rollTop {
              ...
              left: 0px;
              top: 0px;
              overflow: ${showBorder ? 'visible' : 'hidden'};
            ...
            .rollfree-parent${rockName.current}{
              ...
              position: absolute;
              left: -${horizontal ? 50 : 0}px;
              top: -${horizontal ? 0 : 50}px;
